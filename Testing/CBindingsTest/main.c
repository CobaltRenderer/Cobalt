...
static void* LoadPluginModule(const char* rendererDllPath)
{
#ifdef _WIN32
	SetThreadErrorMode(SEM_FAILCRITICALERRORS, NULL);
	HMODULE moduleHandle = LoadLibraryA(rendererDllPath);
	return moduleHandle;
#else
	return dlopen(rendererDllPath, RTLD_NOW);
#endif
}

static void UnloadPluginModule(void* moduleHandle)
{
#ifdef _WIN32
...
static const char* PluginPath(void)
{
#if defined(_WIN32)
	return "VulkanRenderer.dll";
#elif defined(__APPLE__)
	return "libVulkanRenderer.dylib";
#else
	return "libVulkanRenderer.so";
#endif
}
...
{
	// Renderer setup
	const char* pluginPath = PluginPath();
	printf("Using '%s' renderer plugin\n", pluginPath);
	void* moduleHandle = LoadPluginModule(pluginPath);
	...
	REQUIRE(Cobalt_GetRendererPlugin(library, moduleHandle, 0, &plugin) == COBALT_SUCCESS);
	...
	UnloadPluginModule(moduleHandle);
