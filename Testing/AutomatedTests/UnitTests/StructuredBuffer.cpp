...
StructuredBuffer<BufferEntry> bufferData;
...
float4 main(float4 position : SV_POSITION) : SV_TARGET0
{
	...
	float4 myColor = bufferData[indexValue].color;
	return myColor;
}
)";
...
DEFINE_UNIT_TEST_WITH_BASE("Resources/DataArray/StructuredBuffer", UnitTestBase)
{
	...
	std::vector<BufferEntry> inputData;
	...
	size_t structureSize = sizeof(BufferEntry);
	dataArray->SetBufferLayout(structureSize, inputData.size());
	REQUIRE(dataArray->SetInitialData(inputData.data(), inputData.size() * structureSize));
