...
ConsumeStructuredBuffer<BufferEntryIn> bufferDataIn;
AppendStructuredBuffer<BufferEntryOut> bufferDataOut;

float4 main(float4 position : SV_POSITION) : SV_TARGET0
{
	BufferEntryIn inEntry = bufferDataIn.Consume();
	float4 myColor = inEntry.color;
	BufferEntryOut outEntry;
	outEntry.hasEntry = 1;
	outEntry.readIndexValue = inEntry.indexValue;
...
struct BufferEntryIn
{
	...
	unsigned int dummyValue1;
	unsigned int dummyValue2;
	unsigned int dummyValue3;
};
struct BufferEntryOut
{
...
DEFINE_UNIT_TEST_WITH_BASE("Resources/DataArray/ConsumeStructuredBuffer", UnitTestBase)
{
	...
	for (unsigned int posY = 0; posY < windowSizeInPixels.Y(); ++posY)
	{
		...
		{
			unsigned int index = posX + (posY * windowSizeInPixels.X());
			auto& entry = inputData[index];
			entry.color = V4Float32((float)posX / (float)windowSizeInPixels.X(), (float)posY / (float)windowSizeInPixels.Y(), 1.0f, 1.0f);
	...
	dataArrayIn->SetBufferLayout(structureSize, inputData.size(), true);
	dataArrayIn->UpdateCounterResetValue((uint32_t)inputData.size());
	...
	DrawOneFrame();
	session.AddTestImageWithoutReferenceCompare("SquareUnstableGradient", "A square with an unstable color gradient. Results will differ on different runs and different hardware.", std::move(frameBufferCapture));
