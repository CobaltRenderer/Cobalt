...
DEFINE_UNIT_TEST_WITH_BASE("Resources/VertexBuffer/RawVertexAttributeTest", UnitTestBase)
{
	...
	RawVertexAttribute vertexAttributePosition(
	  IVertexAttribute::DataType::Float32, 4, positionVertexData.size(), IVertexAttribute::PerformanceHint::WriteRarely | IVertexAttribute::PerformanceHint::ReadNever, IVertexAttribute::PerformanceHint::WriteNever | IVertexAttribute::PerformanceHint::ReadOften);
	RawVertexAttribute vertexAttributeColor(
	  IVertexAttribute::DataType::Float32, 3, colorVertexData.size(), IVertexAttribute::PerformanceHint::WriteRarely | IVertexAttribute::PerformanceHint::ReadNever, IVertexAttribute::PerformanceHint::WriteNever | IVertexAttribute::PerformanceHint::ReadOften);
	auto vertexBuffer = renderer.CreateVertexBuffer();
	REQUIRE(vertexBuffer->BindVertexAttribute(vertexAttributePosition));
	REQUIRE(vertexAttributePosition.SetInitialData(reinterpret_cast<const uint8_t*>(positionVertexData.data()), 3, sizeof(V4Float32)));
	REQUIRE(vertexBuffer->BindVertexAttribute(vertexAttributeColor));
	REQUIRE(vertexAttributeColor.SetInitialData(reinterpret_cast<const uint8_t*>(colorVertexData.data()), 3, sizeof(V3Float32)));
	...
	// Swap two coordinates, without swapping their color values.
	std::swap(positionVertexData.front(), positionVertexData.back());
	REQUIRE(vertexAttributePosition.QueueDataUpdate(reinterpret_cast<const uint8_t*>(positionVertexData.data()), positionVertexData.size(), 0, sizeof(V4Float32)));

	// And capture the scene with the swapped points
	frameBufferCapture = renderer.CreateFrameBufferOutput();
	frameBufferCapture->SetDetachAfterCapture(true);
	mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("RGBTriangleDownButSwapped", "Two points swapped coordinates (will show as a color flip)", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);

	// Now set the middle colour to white
	V3Float32 white(1, 1, 1);
	REQUIRE(vertexAttributeColor.QueueDataUpdate(reinterpret_cast<const uint8_t*>(&white), 1, 1, sizeof(V3Float32)));
