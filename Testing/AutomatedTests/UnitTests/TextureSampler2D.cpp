...
{
    float4 color = colorTexture.Sample(colorSampler, IN.texCoord.xy);
    return color;
}
)";

DEFINE_UNIT_TEST_WITH_BASE("Resources/ImageSamplers/TextureSampler2D", UnitTestBase)
{
	...
	renderableNode->BindTexture(shaderProgram->GetTextureId("colorTexture"), texture.get());
	renderableNode->BindSampler(shaderProgram->GetSamplerId("colorSampler"), sampler.get());
	...
	DrawOneFrame();
	session.AddTestImageResult("CobaltLogoColor", "Cobalt color logo on a black square using a separate sampler.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("CobaltLogoWhiteStrip", "Cobalt color logo on a black square using a separate sampler, with a white logo strip across the middle.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("CobaltLogoBlackStrip", "Cobalt color logo on a black square using a separate sampler, with a white logo strip across the middle, and black logo on a white background strip on right side.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WrapClampToEdge", "Cobalt logo texture sampled with clamp-to-edge wrap mode using a separate sampler.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WrapRepeat", "Cobalt logo texture sampled with repeat wrap mode using a separate sampler.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WrapRepeatMirrored", "Cobalt logo texture sampled with mirrored-repeat wrap mode using a separate sampler.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("FilterNearest", "A magnified Cobalt logo texture sampled with nearest filtering using a separate sampler.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("FilterLinear", "A magnified Cobalt logo texture sampled with linear filtering using a separate sampler.", std::move(frameBufferCapture), IImageDiff::Algorithm::RegionRanges | IImageDiff::Algorithm::NaiveDiff, 0.95);
