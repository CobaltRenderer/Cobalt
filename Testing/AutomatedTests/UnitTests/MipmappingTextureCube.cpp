...
DEFINE_UNIT_TEST_WITH_BASE("Resources/Mipmapping/TextureCube", UnitTestBase)
{
	...
	// Create our cube texture and populate each mipmap level with deliberately high-contrast face data.
	const uint32_t faceLength = 256;
	auto texture = renderer.CreateTextureBufferCube();
	texture->SetTextureDimensions(faceLength, TestMipmapLevelCount);
	...
	for (int mipmapLevel = 0; mipmapLevel < TestMipmapLevelCount; ++mipmapLevel)
	{
		...
		REQUIRE(texture->SetInitialData(mipLevelData[(size_t)mipmapLevel][0], ITextureBuffer::CubeMapFace::PositiveX, mipmapLevel));
		REQUIRE(texture->SetInitialData(mipLevelData[(size_t)mipmapLevel][1], ITextureBuffer::CubeMapFace::NegativeX, mipmapLevel));
		REQUIRE(texture->SetInitialData(mipLevelData[(size_t)mipmapLevel][2], ITextureBuffer::CubeMapFace::PositiveY, mipmapLevel));
		REQUIRE(texture->SetInitialData(mipLevelData[(size_t)mipmapLevel][3], ITextureBuffer::CubeMapFace::NegativeY, mipmapLevel));
		REQUIRE(texture->SetInitialData(mipLevelData[(size_t)mipmapLevel][4], ITextureBuffer::CubeMapFace::PositiveZ, mipmapLevel));
		REQUIRE(texture->SetInitialData(mipLevelData[(size_t)mipmapLevel][5], ITextureBuffer::CubeMapFace::NegativeZ, mipmapLevel));
	}
	REQUIRE(texture->AllocateMemory());

	// Create a sampler to read the cube texture from the shader.
	...
	DrawOneFrame();
	session.AddTestImageResult("MipmapNone", "A cube texture sampled with mipmapping disabled.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("MipmapNearest", "A cube texture sampled with nearest mipmap selection.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("MipmapLinear", "A cube texture sampled with linear mipmap blending.", std::move(frameBufferCapture), IImageDiff::Algorithm::NaiveDiff, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("MipmapMinLevel", "A cube texture sampled with the minimum mipmap level clamped to level 2.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("MipmapMaxLevel", "A cube texture sampled with the maximum mipmap level clamped to level 1.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	if (mipmapLevelBiasSupported)
	{
		...
		scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("MipmapBiasPositive", "A cube texture sampled with a positive mipmap level bias.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
		...
		scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("MipmapBiasNegative", "A cube texture sampled with a negative mipmap level bias.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	}
	else
	{
	...
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapNone", "A cube texture sampled on curved geometry with mipmapping disabled.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapNearest", "A cube texture sampled on curved geometry with nearest mipmap selection.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapLinear", "A cube texture sampled on curved geometry with linear mipmap blending.", std::move(frameBufferCapture), IImageDiff::Algorithm::NaiveDiff, 0.95);
	...
	perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapMinLevel", "A cube texture sampled on curved geometry with the minimum mipmap level clamped to level 2.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapMaxLevel", "A cube texture sampled on curved geometry with the maximum mipmap level clamped to level 1.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	if (mipmapLevelBiasSupported)
	{
		...
		perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("PerspectiveMipmapBiasPositive", "A cube texture sampled on curved geometry with a positive mipmap level bias.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.90);
		...
		perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("PerspectiveMipmapBiasNegative", "A cube texture sampled on curved geometry with a negative mipmap level bias.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.90);
	}
	else
	{
