...
{
    return colorTexture.Sample(colorSampler, IN.texCoord.x);
}
)";
...
std::vector<V4UInt8> CreateStripData(uint32_t imageWidth)
{
	...
	for (uint32_t posX = 0; posX < imageWidth; ++posX)
	{
		...
		{
			imageData[posX] = V4UInt8(255, 160, 32, 255);
		}
		else if (posX < ((imageWidth * 2) / 3))
		{
			imageData[posX] = V4UInt8(32, 255, 255, 255);
		}
		else
		{
			imageData[posX] = V4UInt8(192, 32, 255, 255);
		}
	}
	return imageData;
}
...
DEFINE_UNIT_TEST_WITH_BASE("Resources/ImageSamplers/TextureSampler1D", UnitTestBase)
{
	...
	DrawOneFrame();
	session.AddTestImageResult("PrimaryColorBands", "Three coloured vertical bands sourced from a 1D texture using a separate sampler.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WhiteMiddleBand", "Three coloured vertical bands sourced from a 1D texture using a separate sampler, with the middle band replaced with white.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WrapClampToEdge", "A 1D texture sampled with a separate sampler using clamp-to-edge wrap mode.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WrapRepeat", "A 1D texture sampled with a separate sampler using repeat wrap mode.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WrapRepeatMirrored", "A 1D texture sampled with a separate sampler using mirrored-repeat wrap mode.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("FilterNearest", "A magnified 1D texture sampled with a separate sampler using nearest filtering.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("FilterLinear", "A magnified 1D texture sampled with a separate sampler using linear filtering.", std::move(frameBufferCapture), IImageDiff::Algorithm::RegionRanges | IImageDiff::Algorithm::NaiveDiff, 0.95);
