...
float4 main(float4 position : SV_POSITION) : SV_TARGET0
{
	...
	bufferDataOut[counterValue].hasEntry = 1;
	bufferDataOut[counterValue].readIndexValue = indexValue;
	bufferDataOut[counterValue].writeIndexValue = counterValue;
	bufferDataOut[counterValue].dummyValue = 0xDEADBEEF;
	bufferDataOut[counterValue].color = myColor;
	return myColor;
}
)";
...
struct BufferEntryOut
{
	...
	unsigned int writeIndexValue;
	unsigned int dummyValue;
	V4Float32 color;
};
} // namespace

DEFINE_UNIT_TEST_WITH_BASE("Resources/DataArray/RWStructuredBuffer", UnitTestBase)
{
	...
	for (size_t i = 0; i < counterValue; ++i)
	{
		...
		REQUIRE(entry.writeIndexValue == i);
