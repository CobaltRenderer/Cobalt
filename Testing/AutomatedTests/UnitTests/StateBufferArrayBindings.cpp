...
DEFINE_UNIT_TEST_WITH_BASE("Resources/StateBuffer/StateBufferArrayBindings", UnitTestBase)
{
	...
	auto stateBufferId = shaderProgram->GetStateBufferId("MyStateBuffer");
	...
	auto stateValueIdMulLevel1End = stateBuffer->GetStateValueId("level1.mulLevel1End");
	stateBuffer->SetStateValue(stateValueIdMulLevel1End, V1Float32(2.0f));
	auto stateValueIdMultidimensionalArrayFlattened = stateBuffer->GetStateValueId("level1.multiDimensionalArray[]");
	stateBuffer->SetStateValue(stateValueIdMultidimensionalArrayFlattened, V1Float32(1.0f), (1 * (4 * 6)) + (2 * 6) + 3);
	auto stateValueIdMulLevel2End = stateBuffer->GetStateValueId("level1.level2[].mulLevel2End");
	stateBuffer->SetStateValue(stateValueIdMulLevel2End, V1Float32(0.5f), 0);
	stateBuffer->SetStateValue(stateValueIdMulLevel2End, V1Float32(0.25f), 2);
	stateBuffer->SetStateValue(stateValueIdMulLevel2End, V1Float32(0.5f), 1);
	auto stateValueId = stateBuffer->GetStateValueId("level1.level2[].level3[].value[]");
	stateBuffer->SetStateValue(stateValueId, V1Float32(1.0f), 1, 0, 0);
	stateBuffer->SetStateValue(stateValueId, V1Float32(0.0f), 3, 1, 0);
	stateBuffer->SetStateValue(stateValueId, V1Float32(0.0f), 5, 2, 0);
	stateBuffer->SetStateValue(stateValueId, V1Float32(0.0f), 1, 0, 1);
	stateBuffer->SetStateValue(stateValueId, V1Float32(2.0f), 3, 1, 1);
	stateBuffer->SetStateValue(stateValueId, V1Float32(0.0f), 5, 2, 1);
	stateBuffer->SetStateValue(stateValueId, V1Float32(0.0f), 1, 0, 2);
	stateBuffer->SetStateValue(stateValueId, V1Float32(0.0f), 3, 1, 2);
	stateBuffer->SetStateValue(stateValueId, V1Float32(1.0f), 5, 2, 2);

	// Bind the state buffer to the renderable
	renderableNode->BindStateBuffer(stateBufferId, stateBuffer.get());
