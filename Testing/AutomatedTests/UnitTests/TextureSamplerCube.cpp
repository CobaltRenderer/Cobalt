...
{
    return colorTexture.Sample(colorSampler, GetCubeDirection(IN.texCoord.xy, faceIndex));
}
)";
...
float4 main(VSOutput IN) : SV_TARGET0
{
    return colorTexture.Sample(colorSampler, normalize(IN.sampleDirection));
}
)";
...
DEFINE_UNIT_TEST_WITH_BASE("Resources/ImageSamplers/TextureSamplerCube", UnitTestBase)
{
	...
	DrawOneFrame();
	session.AddTestImageResult("PositiveXFace", "A cube texture positive X face with warm vertical bands using a separate sampler.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("NegativeZFace", "A cube texture negative Z face with cool vertical bands using a separate sampler.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("NegativeZFaceWhiteMiddle", "A cube texture negative Z face with cool vertical bands using a separate sampler, with a white strip through the middle.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WrapClampToEdge", "A cube texture sampled with a separate sampler using clamp-to-edge wrap mode.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WrapRepeat", "A cube texture sampled with a separate sampler using repeat wrap mode.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("WrapRepeatMirrored", "A cube texture sampled with a separate sampler using mirrored-repeat wrap mode.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("FilterNearest", "A magnified cube texture sampled with a separate sampler using nearest filtering.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("FilterLinear", "A magnified cube texture sampled with a separate sampler using linear filtering.", std::move(frameBufferCapture), IImageDiff::Algorithm::RegionRanges | IImageDiff::Algorithm::NaiveDiff, 0.95);
	...
	// Define a curved display scene so we can verify that the cubemap and separate sampler work correctly across a
	// realistic spread of sample directions.
	...
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveDiagonalNearest", "A cube texture sampled on a diagonally viewed sphere using a separate sampler with nearest filtering.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveDiagonalLinear", "A cube texture sampled on a diagonally viewed sphere using a separate sampler with linear filtering.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
