...
DEFINE_UNIT_TEST_WITH_BASE("Resources/Batching/DataArrayTransferBatch", UnitTestBase)
{
	...
	colorArray->SetBufferLayout(sizeof(V4Float32), baseColors.size());
	colorArray->SetUsageFlags(IDataArray::UsageFlags::ShaderInput | IDataArray::UsageFlags::TransferDestination);
	REQUIRE(colorArray->SetInitialData(baseColors.data(), baseColors.size() * sizeof(V4Float32)));
	REQUIRE(colorArray->AllocateMemory());

	auto transferArray = renderer.CreateDataArray();
	transferArray->SetBufferLayout(sizeof(V4Float32), transferColors.size());
	transferArray->SetUsageFlags(IDataArray::UsageFlags::TransferSource);
	REQUIRE(transferArray->SetInitialData(transferColors.data(), transferColors.size() * sizeof(V4Float32)));
	...
	for (const auto& config : transferBatchConfigs)
	{
		...
		REQUIRE(colorArray->QueueDataUpdate(baseColors.data(), baseColors.size() * sizeof(V4Float32)));
		...
		REQUIRE(colorArray->QueueDataUpdate(&updatedMiddleColor, sizeof(updatedMiddleColor), sizeof(V4Float32), transferBatch.get()));
		REQUIRE(transferArray->QueueDataTransfer(colorArray.get(), sizeof(V4Float32), 0, 0, transferBatch.get()));
		REQUIRE(transferArray->QueueDataTransfer(colorArray.get(), sizeof(V4Float32), sizeof(V4Float32), sizeof(V4Float32) * 2, transferBatch.get()));
		...
		mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("Transfer" + configName + "Deferred", "The batched data transfer has not been submitted yet, so the red, green and blue bands should be unchanged.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
		...
		mainWindowFrameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("Transfer" + configName + "Submitted", "The batched data transfer has completed, producing cyan, magenta and yellow vertical bands.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
		REQUIRE(transferBatch->IsComplete());
	}
