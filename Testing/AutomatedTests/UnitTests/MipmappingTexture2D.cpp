...
DEFINE_UNIT_TEST_WITH_BASE("Resources/Mipmapping/Texture2D", UnitTestBase)
{
	...
	// Create our texture and populate each mipmap level with deliberately high-contrast quadrant data.
	...
	DrawOneFrame();
	session.AddTestImageResult("MipmapNone", "A 2D texture sampled with mipmapping disabled.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("MipmapNearest", "A 2D texture sampled with nearest mipmap selection.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("MipmapLinear", "A 2D texture sampled with linear mipmap blending.", std::move(frameBufferCapture), IImageDiff::Algorithm::NaiveDiff, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("MipmapMinLevel", "A 2D texture sampled with the minimum mipmap level clamped to level 2.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("MipmapMaxLevel", "A 2D texture sampled with the maximum mipmap level clamped to level 1.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	if (mipmapLevelBiasSupported)
	{
		...
		scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("MipmapBiasPositive", "A 2D texture sampled with a positive mipmap level bias.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
		...
		scene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("MipmapBiasNegative", "A 2D texture sampled with a negative mipmap level bias.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	}
	else
	{
	...
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapNone", "A 2D texture sampled on a perspective floor plane with mipmapping disabled.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapNearest", "A 2D texture sampled on a perspective floor plane with nearest mipmap selection.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapLinear", "A 2D texture sampled on a perspective floor plane with linear mipmap blending.", std::move(frameBufferCapture), IImageDiff::Algorithm::NaiveDiff, 0.90);
	...
	perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapMinLevel", "A 2D texture sampled on a perspective floor plane with the minimum mipmap level clamped to level 2.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveMipmapMaxLevel", "A 2D texture sampled on a perspective floor plane with the maximum mipmap level clamped to level 1.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.95);
	...
	if (mipmapLevelBiasSupported)
	{
		...
		perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("PerspectiveMipmapBiasPositive", "A 2D texture sampled on a perspective floor plane with a positive mipmap level bias.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.90);
		...
		perspectiveScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult("PerspectiveMipmapBiasNegative", "A 2D texture sampled on a perspective floor plane with a negative mipmap level bias.", std::move(frameBufferCapture), IImageDiff::Algorithm::AllOfThem, 0.90);
	}
	else
	{
	...
	// Create a texture from the loaded floor image and upload every generated mipmap level.
	auto floorTexture = renderer.CreateTextureBuffer2D();
	floorTexture->SetTextureDimensions(floorImage->size, floorImage->mipmapLevelCount);
	...
	floorTexture->SetPerformanceHints(ITextureBuffer::PerformanceHint::WriteNever | ITextureBuffer::PerformanceHint::ReadNever, ITextureBuffer::PerformanceHint::WriteNever | ITextureBuffer::PerformanceHint::ReadOften);
	for (uint32_t mipmapLevel = 0; mipmapLevel < floorImage->mipmapLevelCount; ++mipmapLevel)
	{
		REQUIRE(floorTexture->SetInitialData(floorImage->mipmapTextureData[(size_t)mipmapLevel], (int)mipmapLevel));
	}
	...
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveFloorNearest", "A tiled floor texture sampled with nearest filtering on a perspective plane.", std::move(frameBufferCapture), IImageDiff::Algorithm::NaiveDiff, 0.95);
	...
	floorScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
	DrawOneFrame();
	session.AddTestImageResult("PerspectiveFloorLinear", "A tiled floor texture sampled with linear filtering on a perspective plane.", std::move(frameBufferCapture), IImageDiff::Algorithm::NaiveDiff, 0.95);
	...
	if (session.Device().IsFeatureSupported(IGraphicsDevice::Feature::AnisotropicFiltering))
	{
		...
		for (int anisotropyLevel = 2; anisotropyLevel <= maxAnisotropyLevel; anisotropyLevel *= 2)
		{
			...
			floorScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
			DrawOneFrame();
			session.AddTestImageResult("PerspectiveFloorAnisotropic" + std::to_string(anisotropyLevel) + "x", "A tiled floor texture sampled with anisotropic filtering level " + std::to_string(anisotropyLevel) + " on a perspective plane.", std::move(frameBufferCapture), IImageDiff::Algorithm::NaiveDiff, 0.95);
		...
		if (!maxLevelCapturedExplicitly)
		{
			...
			floorScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
			DrawOneFrame();
			session.AddTestImageResult(autoMaxTestName, "A tiled floor texture sampled with anisotropic filtering level " + std::to_string(maxAnisotropyLevel) + " on a perspective plane.", std::move(frameBufferCapture), IImageDiff::Algorithm::NaiveDiff, 0.95);
		}
		...
		floorScene.frameBuffer->AddOutputCaptureTarget(frameBufferCapture.get(), IFrameBuffer::AttachmentType::Color);
		DrawOneFrame();
		session.AddTestImageResult(autoMaxTestName, "A tiled floor texture sampled with auto-max anisotropic filtering, which should match anisotropic filtering level " + std::to_string(maxAnisotropyLevel) + " on a perspective plane.", std::move(frameBufferCapture), IImageDiff::Algorithm::NaiveDiff, 0.95);
	}
	else
	{
