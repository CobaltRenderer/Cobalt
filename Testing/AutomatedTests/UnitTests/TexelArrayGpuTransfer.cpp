...
DEFINE_UNIT_TEST_WITH_BASE("Resources/TexelArray/GPUTransfer", UnitTestBase)
{
	...
	colorArray->SetBufferLayout(ITexelArray::ImageFormat::RGBA, ITexelArray::DataFormat::Float32, colorData.size());
	colorArray->SetUsageFlags(ITexelArray::UsageFlags::ShaderInput | ITexelArray::UsageFlags::TransferDestination);
	REQUIRE(colorArray->SetInitialData(colorData));
	REQUIRE(colorArray->AllocateMemory());
	auto altColorArray = renderer.CreateTexelArray();
	altColorArray->SetBufferLayout(ITexelArray::ImageFormat::RGBA, ITexelArray::DataFormat::Float32, altColorData.size());
	altColorArray->SetUsageFlags(ITexelArray::UsageFlags::TransferSource);
	REQUIRE(altColorArray->SetInitialData(altColorData));
	...
	REQUIRE(altColorArray->QueueDataTransfer(colorArray.get(), 1, 0, 2));
	...
	// Queue a transfer overwriting an entry in the color array with an entry from our alternate color array
	REQUIRE(altColorArray->QueueDataTransfer(colorArray.get(), 1, 1, 1));
	...
	// Queue a transfer overwriting an entry in the color array with an entry from our alternate color array
	REQUIRE(altColorArray->QueueDataTransfer(colorArray.get(), 1, 2, 0));
