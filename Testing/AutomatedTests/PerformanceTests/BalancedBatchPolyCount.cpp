...
DEFINE_UNIT_TEST_WITH_BASE("Performance/BalancedBatchPolyCount", PerformanceTestBase)
{
	...
	REQUIRE(shaderProgram->LoadShaderStage(IShaderProgram::ShaderStage::Vertex, ShaderSourceInfoHLSL(VertexShader.data())));
	REQUIRE(shaderProgram->LoadShaderStage(IShaderProgram::ShaderStage::Fragment, ShaderSourceInfoHLSL(FragmentShader.data())));
	...
	//##FIX## This comment is wrong
	// Create approximately 100000 sphere objects with 98 triangles per sphere, with separate buffers and renderable
	// objects, and unique per-object state. This will stress test the ability of the main render loop to process and
	// dispatch a large number of primitives per frame, and handle a large volume of state changes mid frame. This is
	// ideal for helping optimise the innermost level of the main render loop, and the draw function of the renderable
	// node.
	size_t primitiveCount = 10000;
	uint32_t vertexCountWidth = 71;
	uint32_t vertexCountHeight = 71;
	...
	session.AddTestProfileResult("BalancedBatchPolyCount", "10000 spheres with 10082 triangles each", profileResults);
