...
DEFINE_UNIT_TEST_WITH_BASE("Performance/ConstantFrameCaptureBuffer", PerformanceTestBase)
{
	...
	REQUIRE(testWindowFrameBuffer->BindTexture(opaqueColorTexture.get(), IFrameBuffer::AttachmentType::Color));
	REQUIRE(testWindowFrameBuffer->BindTexture(depthTexture.get(), IFrameBuffer::AttachmentType::Depth));
	...
	session.AddTestProfileResult("ConstantFrameCaptureBuffer", "A single triangle in a single render pass", profileResults);
