...
public:
	// Constructors
	inline InOut(ContainerType& target) // NOLINT(hicpp-explicit-conversions)
	: _sourceReference(_source), _targetReference(_target), _source(target), _target(target)
	{
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
		static_assert(!marshalling::internal::is_only_movable<ContainerType>::value, "Target of Marshal::InOut can't be move-only");
		static_assert(marshalling::internal::is_assignable<ContainerType>::value, "Target of Marshal::InOut must be assignable");
#endif
	}
	...
	inline const InOut& operator=(const ContainerType& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(sourceObject);
		return *this;
	}
	...
	inline const InOut& operator=(ContainerType&& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(std::move(sourceObject));
		return *this;
	}
...
protected:
	...
	// constructor is to introduce a path by which the _sourceReference and _targetReference members could be
	// initialized with a value of anything other than the addresses of the _source and _target members. Since this type
	// could be derived from elsewhere, and this constructor could be called by a derived type, and we explicitly set
	// the value of _sourceReference and _targetReference here to an externally provided value, the optimizer cannot
	// assume what the contents of these members could be based on the other provided constructors, making it impossible
	// to optimize away virtual function calls within the referenced object.
...
private:
	...
	InOut(const InOut& sourceObject) MARSHALSUPPORT_DELETEMETHOD;      // NOLINT(hicpp-use-equals-delete,modernize-use-equals-delete)
	InOut& operator=(const InOut& source) MARSHALSUPPORT_DELETEMETHOD; // NOLINT(hicpp-use-equals-delete,modernize-use-equals-delete)
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	InOut(InOut&& sourceObject) MARSHALSUPPORT_DELETEMETHOD;      // NOLINT(hicpp-use-equals-delete,modernize-use-equals-delete)
	InOut& operator=(InOut&& source) MARSHALSUPPORT_DELETEMETHOD; // NOLINT(hicpp-use-equals-delete,modernize-use-equals-delete)
#endif
...
public:
	// Constructors
	inline InOut(std::vector<ElementType, Alloc>& target) // NOLINT(hicpp-explicit-conversions)
	: _sourceReference(_source), _targetReference(_target), _source(target), _target(target)
	{}
	...
	inline const InOut& operator=(const std::vector<ElementType, Alloc>& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(sourceObject);
		return *this;
	}
	...
	inline const InOut& operator=(std::vector<ElementType, Alloc>&& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(std::move(sourceObject));
		return *this;
	}
	...
	// Implicit conversions
	inline operator std::vector<ElementType, Alloc>() const // NOLINT(hicpp-explicit-conversions)
	{
		return _sourceReference.MarshalToWithoutMove();
	}
...
public:
	// Constructors
	inline InOut(std::array<ElementType, ArraySize>& target) // NOLINT(hicpp-explicit-conversions)
	: _sourceReference(_source), _targetReference(_target), _source(target), _target(target)
	{}
	...
	inline const InOut& operator=(const std::array<ElementType, ArraySize>& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(sourceObject);
		return *this;
	}
	...
	inline const InOut& operator=(std::array<ElementType, ArraySize>&& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(std::move(sourceObject));
		return *this;
	}
	...
	// Implicit conversions
	inline operator std::array<ElementType, ArraySize>() const // NOLINT(hicpp-explicit-conversions)
	{
		return _sourceReference.MarshalToWithoutMove();
	}
