...
void DecomposeSTLContainer(void* itemArray, size_t elementSizeArray[], INestedMarshallerBase* const nestedMarshallerArray[], size_t& elementArrayIndex, size_t& elementSizeArrayIndex, size_t nestedMarshallerArrayIndex, const std::vector<ElementType, Alloc>& element)
{
	...
	if (!has_marshal_constructor<ElementType>::value && std::is_trivial<ElementType>::value)
	{
		...
		std::memcpy((void*)&itemArrayWithType[elementArrayIndex], (const void*)element.data(), containerEntryCount * sizeof(ElementType));
...
{
	// Note that we specifically exclude vectors from using memcpy here, since we can't do that on the bit-packed
	// specialization of a boolean vector that's still part of the C++ standard at this time, and the data() method
	// doesn't exist on this container specialization either, so we can't use type traits within the generic vector
	// specialization to exclude it.
	size_t containerEntryCount = element.size();
	elementSizeArray[elementSizeArrayIndex++] = containerEntryCount;
	bool* itemArrayWithType = reinterpret_cast<bool*>(itemArray);
	for (size_t i = 0; i < containerEntryCount; ++i)
	{
...
void DecomposeSTLContainer(void* itemArray, size_t elementSizeArray[], INestedMarshallerBase* const nestedMarshallerArray[], size_t& elementArrayIndex, size_t& elementSizeArrayIndex, size_t nestedMarshallerArrayIndex, const std::array<ElementType, ArraySize>& element)
{
	...
	{
		ElementType* itemArrayWithType = (ElementType*)itemArray;
		std::memcpy((void*)&itemArrayWithType[elementArrayIndex], (const void*)element.data(), ArraySize * sizeof(ElementType));
...
void DecomposeSTLContainer(void* itemArray, size_t elementSizeArray[], INestedMarshallerBase* const nestedMarshallerArray[], size_t& elementArrayIndex, size_t& elementSizeArrayIndex, size_t nestedMarshallerArrayIndex, std::set<ElementType, Compare, Alloc>&& element)
{
	...
	for (typename std::set<ElementType, Compare, Alloc>::const_iterator i = element.begin(); i != element.end(); ++i)
	{
		// Note that we can't use a non-const iterator and move the elements from this container, because as per the C++
		// standard 23.2.4/6 iterator and const_iterator are both const iterators for set containers. Casting away the
		// const property and performing a move here during iteration would cause undefined behaviour.
		DecomposeSTLContainer(itemArray, elementSizeArray, nestedMarshallerArray, elementArrayIndex, elementSizeArrayIndex, nestedMarshallerArrayIndex + 1, *i);
		++containerEntryCount;
	}
	elementSizeArray[elementSizeArrayIndexForContainer] = containerEntryCount;
...
void DecomposeSTLContainer(void* itemArray, size_t elementSizeArray[], INestedMarshallerBase* const nestedMarshallerArray[], size_t& elementArrayIndex, size_t& elementSizeArrayIndex, size_t nestedMarshallerArrayIndex, const std::priority_queue<ElementType, Container, Compare>& element)
{
	...
	for (size_t i = 0; i < elementSize; ++i)
	{
		// Note that the double copy here is inefficient, but due to an oversight in the current design of std::priority_queue,
...
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	static_assert(is_last_nested_container_element<ElementType>::value, "Marshalling library internal error. DecomposeSTLContainer reached final element marshaller with a type that still contains nested elements.");
#else
...
{
	// If a non-const lvalue reference has been supplied, convert it to a const lvalue reference and resolve the
	// function again. We only supply overloads on this function for STL container types using const lvalue and
	// non-const rvalue references, so we need to ensure that non-const lvalue references are made const before they are
	// handled.
...
void DecomposeSTLContainer(void* itemArray, size_t elementSizeArray[], INestedMarshallerBase* const nestedMarshallerArray[], size_t& elementArrayIndex, size_t& elementSizeArrayIndex, size_t nestedMarshallerArrayIndex, ElementType&& element)
{
	...
	// in this case.
	static_assert(!std::is_const<ElementType>::value, "Marshalling library internal error. DecomposeSTLContainer was called with a const rvalue reference.");

	// Ensure that this function hasn't been called when there are still STL containers we need to decompose
	static_assert(is_last_nested_container_element<typename std::decay<ElementType>::type>::value, "Marshalling library internal error. DecomposeSTLContainer reached final element marshaller with a type that still contains nested elements.");
