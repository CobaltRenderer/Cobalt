...
// prior to its use with the marshalling library.
template<class T, bool isDefined = is_complete<T>::value>
struct is_only_movable_helper
{};
template<class T>
struct is_only_movable_helper<T, true>
{
	static const bool value = std::integral_constant < bool, (std::is_move_assignable<T>::value && !std::is_copy_assignable<T>::value) || (std::is_move_constructible<T>::value && !std::is_copy_constructible<T>::value) > ::value;
};
template<class T>
struct is_only_movable_helper<T, false>
{
...
{
	static const bool value = std::integral_constant < bool, is_only_movable<T>::value || is_only_movable<Args...>::value > ::value;
};
template<class T>
struct is_only_movable<T>
{
	static const bool value = std::integral_constant<bool, is_only_movable_helper<T>::value>::value;
};
#else
template<class T>
struct is_only_movable
...
struct is_only_movable<std::pair<T1, T2>>
{
	static const bool value = std::integral_constant < bool, is_only_movable<T1>::value || is_only_movable<T2>::value > ::value;
};
#ifndef MARSHALSUPPORT_NOVARIADICTEMPLATES
template<class... Args>
struct is_only_movable<std::tuple<Args...>>
{
	static const bool value = is_only_movable<Args...>::value;
};
template<>
struct is_only_movable<std::tuple<>>
{
	static const bool value = false;
};
#endif
template<class T, class Deleter>
struct is_only_movable<std::unique_ptr<T, Deleter>>
{
	static const bool value = false;
};
#else
