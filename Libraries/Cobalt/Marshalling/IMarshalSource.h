...
template<class ContainerType, bool IsOnlyMovable = marshalling::internal::is_only_movable<typename marshalling::internal::get_last_nested_container_element_type<ContainerType>::type>::value, bool IsAssignable = marshalling::internal::is_assignable<ContainerType>::value>
...
protected:
	...
	// Marshalling methods
	virtual void MARSHALSUPPORT_CALLINGCONVENTION RetrieveData(size_t& elementByteSize, void*& itemArray, const size_t*& elementSizeArray, marshalling::internal::INestedMarshallerBase* const*& nestedMarshallerArray, bool allowMove) const = 0;
	virtual void MARSHALSUPPORT_CALLINGCONVENTION FreeDataArrays(void* itemArray, const size_t* elementSizeArray, marshalling::internal::INestedMarshallerBase* const* nestedMarshallerArray) const = 0;
...
private:
	...
	public:
		// Constructors
		explicit ScopedFreeDataArrayHelper(const IMarshalSourceInternal& atargetMarshaller)
	...
	public:
		...
		marshalling::internal::INestedMarshallerBase* const* nestedMarshallerArray;
	};
...
private:
	...
	{
		// Retrieve arrays to load the decomposed object data from. Note that we give ownership of the arrays to a
		// helper object which frees the data in its destructor, to ensure we don't leak memory if an exception is
		// triggered during the marshalling process.
		...
		marshalling::internal::EnsureSTLContainerKeySizesMatch(arrayHelper.nestedMarshallerArray, 0, (ContainerType*)0);
		...
		marshalling::internal::RecomposeSTLContainer(elementByteSize, arrayHelper.itemArray, arrayHelper.elementSizeArray, arrayHelper.nestedMarshallerArray, elementArrayIndex, elementSizeArrayIndex, 0, targetObject);
	...
	void MarshalToInternal(ContainerType& targetObject) const
	{
		...
		marshalling::internal::ClearSTLContainer(targetObject);
...
protected:
	...
	// Marshalling methods
	virtual void MARSHALSUPPORT_CALLINGCONVENTION RetrieveData(size_t& elementByteSize, void*& itemArray, const size_t*& elementSizeArray, marshalling::internal::INestedMarshallerBase* const*& nestedMarshallerArray) const = 0;
	virtual void MARSHALSUPPORT_CALLINGCONVENTION FreeDataArrays(void* itemArray, const size_t* elementSizeArray, marshalling::internal::INestedMarshallerBase* const* nestedMarshallerArray) const = 0;
...
// this specialization will cover all use of STL containers, including containers nested within other containers,
// however there are some further specializations below to optimize marshalling performance for various STL container
// types where the contained elements are directly assignable or marshallable. Also note we don't actually do any direct
...
class IMarshalSource<ContainerType, false, IsThisOrNextElementLastElement, IsOnlyMovable> : public marshalling::internal::IMarshalSourceInternal<ContainerType, IsOnlyMovable>
{};

//----------------------------------------------------------------------------------------
// Specialize IMarshalSource for the case that the source object is directly assignable or marshallable, and where a
// move operation is not strictly required.
template<class ContainerType>
...
private:
	...
	ContainerType MarshalToInternal() const
	{
		...
		{
			return marshalling::internal::MarshalObjectHelper<ContainerType>::MarshalObjectToNewObject(MARSHALSUPPORT_MOVE(const_cast<ContainerType&>(*sourceData)));
		}
		else
		{
			return marshalling::internal::MarshalObjectHelper<ContainerType>::MarshalObjectToNewObject(*sourceData);
		}
	}
	template<bool AllowMove>
	void MarshalToInternal(ContainerType& targetObject) const
	{
		...
		if (sourceDataMovable && AllowMove)
		{
			marshalling::internal::MarshalObjectHelper<ContainerType>::MarshalObjectToExistingObject(MARSHALSUPPORT_MOVE(const_cast<ContainerType&>(*sourceData)), targetObject);
		}
		else
		{
			marshalling::internal::MarshalObjectHelper<ContainerType>::MarshalObjectToExistingObject(*sourceData, targetObject);
		}
	}
};
...
public:
	...
	inline ContainerType MarshalTo() const
	{
		...
		// Marshal the object, performing a move, copy, or invoking a marshal constructor as required.
		return marshalling::internal::MarshalObjectHelper<ContainerType>::MarshalObjectToNewObject(std::move(*sourceData));
	}
	inline void MarshalTo(ContainerType& targetObject) const
	{
		// Retrieve a reference to the object being marshalled
		...
		// Marshal the object, performing a move, copy, or invoking a marshal constructor as required.
		marshalling::internal::MarshalObjectHelper<ContainerType>::MarshalObjectToExistingObject(std::move(*sourceData), targetObject);
	}
...
public:
	...
	inline void GetElement(typename std::vector<ElementType, Alloc>::size_type index, ElementType& element) const
	{
		...
		// that of the sender. To account for this, we calculate the address of each object in the array using the
		// object byte size specified by the sender.
		...
		marshalling::internal::MarshalObjectHelper<ElementType>::MarshalObjectToExistingObject(*sourceObject, element);
	}
	...
	void MarshalToInternal(std::vector<ElementType, Alloc>& targetObject) const
	{
		...
		if (marshalling::internal::has_marshal_constructor<ElementType>::value)
		{
			...
			if (sourceDataMovable && AllowMove)
			{
				...
				{
					marshalling::internal::MarshalObjectHelper<std::vector<ElementType, Alloc>>::MarshalObjectToContainer(MARSHALSUPPORT_MOVE(const_cast<ElementType&>(*nextSourceObject)), targetObject);
			...
			else
			{
				for (size_t i = 0; i < sourceDataLength; ++i)
				{
					marshalling::internal::MarshalObjectHelper<std::vector<ElementType, Alloc>>::MarshalObjectToContainer(*nextSourceObject, targetObject);
					nextSourceObject = reinterpret_cast<const ElementType*>(reinterpret_cast<const unsigned char*>(nextSourceObject) + elementByteSize);
				}
			}
		}
...
private:
	...
	void MarshalToInternal(std::vector<ElementType, Alloc>& targetObject) const
	{
		...
		if (marshalling::internal::has_marshal_constructor<ElementType>::value)
		{
			...
			for (size_t i = 0; i < sourceDataLength; ++i)
			{
				marshalling::internal::MarshalObjectHelper<std::vector<ElementType, Alloc>>::MarshalObjectToContainer(std::move(*nextSourceObject), targetObject);
...
public:
	...
	// Capacity methods
	inline typename std::vector<bool, Alloc>::size_type size() const
	{
		return static_cast<typename std::vector<bool, Alloc>::size_type>(GetSize());
	}
	inline typename std::vector<bool, Alloc>::size_type capacity() const
	{
		return static_cast<typename std::vector<bool, Alloc>::size_type>(GetReservedSize());
	}
	inline bool empty() const
	{
		return (GetSize() == 0);
	...
	// Element access methods
	inline void GetElement(typename std::vector<bool, Alloc>::size_type index, bool& element) const
	{
		RetrieveElement(static_cast<size_t>(index), element);
	}
...
protected:
	...
	virtual void MARSHALSUPPORT_CALLINGCONVENTION RetrieveElement(size_t elementIndex, bool& element) const = 0;
...
public:
	...
	inline void GetElement(typename std::array<ElementType, ArraySize>::size_type index, ElementType& element) const
	{
		...
		marshalling::internal::MarshalObjectHelper<ElementType>::MarshalObjectToExistingObject(*sourceElement, element);
	}
	...
	void MarshalToInternal(std::array<ElementType, ArraySize>& targetObject) const
	{
		...
		if (marshalling::internal::has_marshal_constructor<ElementType>::value)
		{
			...
			if (sourceDataMovable && AllowMove)
			{
				...
				{
					marshalling::internal::MarshalObjectHelper<ElementType>::MarshalObjectToExistingObject(std::move(const_cast<ElementType&>(*nextSourceObject)), targetObject[(typename std::array<ElementType, ArraySize>::size_type)i]);
					nextSourceObject = reinterpret_cast<const ElementType*>(reinterpret_cast<const unsigned char*>(nextSourceObject) + elementByteSize);
				}
			}
			else
			{
				for (size_t i = 0; i < ArraySize; ++i)
				{
					marshalling::internal::MarshalObjectHelper<ElementType>::MarshalObjectToExistingObject(*nextSourceObject, targetObject[(typename std::array<ElementType, ArraySize>::size_type)i]);
					nextSourceObject = reinterpret_cast<const ElementType*>(reinterpret_cast<const unsigned char*>(nextSourceObject) + elementByteSize);
				}
			}
		}
		else
		{
			...
			{
				std::memcpy((void*)targetObject.data(), (const void*)sourceData, ArraySize * sizeof(ElementType));
			}
			else if (sourceDataMovable && AllowMove)
			{
				for (size_t i = 0; i < ArraySize; ++i)
				{
					marshalling::internal::MarshalObjectHelper<ElementType>::MarshalObjectToExistingObject(std::move(const_cast<ElementType&>(sourceData[i])), targetObject[(typename std::array<ElementType, ArraySize>::size_type)i]);
				}
			}
			else
			{
				for (size_t i = 0; i < ArraySize; ++i)
				{
					marshalling::internal::MarshalObjectHelper<ElementType>::MarshalObjectToExistingObject(sourceData[i], targetObject[(typename std::array<ElementType, ArraySize>::size_type)i]);
				}
			}
		}
	}
...
public:
	...
	inline void GetElement(typename std::array<ElementType, ArraySize>::size_type index, ElementType& element) const
	{
		...
		marshalling::internal::MarshalObjectHelper<ElementType>::MarshalObjectToExistingObject(std::move(*sourceElement), element);
	}
...
private:
	...
	void MarshalToInternal(std::array<ElementType, ArraySize>& targetObject) const
	{
		...
		if (marshalling::internal::has_marshal_constructor<ElementType>::value)
		{
			...
			for (size_t i = 0; i < ArraySize; ++i)
			{
				marshalling::internal::MarshalObjectHelper<ElementType>::MarshalObjectToExistingObject(std::move(*nextSourceObject), targetObject[(typename std::array<ElementType, ArraySize>::size_type)i]);
				nextSourceObject = reinterpret_cast<ElementType*>(reinterpret_cast<unsigned char*>(nextSourceObject) + elementByteSize);
			}
		}
		else
		{
			...
			for (size_t i = 0; i < ArraySize; ++i)
			{
				marshalling::internal::MarshalObjectHelper<ElementType>::MarshalObjectToExistingObject(std::move(sourceData[i]), targetObject[(typename std::array<ElementType, ArraySize>::size_type)i]);
			}
		}
	}
};
	...
	void MarshalToInternal(std::basic_string<ElementType, traits, Alloc>& targetObject) const
	{
		...
		if (marshalling::internal::has_marshal_constructor<ElementType>::value)
		{
			...
			if (sourceDataMovable && AllowMove)
			{
				for (size_t i = 0; i < sourceDataLength; ++i)
				{
					marshalling::internal::MarshalObjectHelper<std::basic_string<ElementType, traits, Alloc>>::MarshalObjectToContainer(MARSHALSUPPORT_MOVE(const_cast<ElementType&>(*nextSourceObject)), targetObject);
					nextSourceObject = reinterpret_cast<const ElementType*>(reinterpret_cast<const unsigned char*>(nextSourceObject) + elementByteSize);
				}
			}
			else
			{
				for (size_t i = 0; i < sourceDataLength; ++i)
				{
					marshalling::internal::MarshalObjectHelper<std::basic_string<ElementType, traits, Alloc>>::MarshalObjectToContainer(*nextSourceObject, targetObject);
					nextSourceObject = reinterpret_cast<const ElementType*>(reinterpret_cast<const unsigned char*>(nextSourceObject) + elementByteSize);
				}
			}
		}
...
public:
	...
	{
		marshalling::internal::INestedMarshallerBase* elementMarshallers[std::tuple_size<std::tuple<Args...>>::value];
...
protected:
	...
	// Marshalling methods
	virtual void MARSHALSUPPORT_CALLINGCONVENTION CreateElementMarshallers(marshalling::internal::INestedMarshallerBase* (&elementMarshallers)[std::tuple_size<std::tuple<Args...>>::value], bool allowMove) const = 0;
	virtual void MARSHALSUPPORT_CALLINGCONVENTION DeleteElementMarshallers(marshalling::internal::INestedMarshallerBase* (&elementMarshallers)[std::tuple_size<std::tuple<Args...>>::value]) const = 0;
...
private:
	// Marshalling methods
	template<int... S>
	static std::tuple<Args...> MakeTupleFromElementMarshallers(IntegerSequence<S...>, marshalling::internal::INestedMarshallerBase* (&elementMarshallers)[std::tuple_size<std::tuple<Args...>>::value])
	{
		return std::make_tuple(((marshalling::internal::INestedMarshaller<typename std::tuple_element<S, std::tuple<Args...>>::type>*)elementMarshallers[S])->RemoveKey(sizeof(typename std::tuple_element<S, std::tuple<Args...>>::type))...);
	}
};
...
protected:
	...
	// Marshalling methods
	virtual void MARSHALSUPPORT_CALLINGCONVENTION CreateElementMarshallers(marshalling::internal::INestedMarshallerBase* (&elementMarshallers)[std::tuple_size<std::tuple<Args...>>::value]) const = 0;
	virtual void MARSHALSUPPORT_CALLINGCONVENTION DeleteElementMarshallers(marshalling::internal::INestedMarshallerBase* (&elementMarshallers)[std::tuple_size<std::tuple<Args...>>::value]) const = 0;
