...
public:
	...
	// Note that we make our nested object protected rather than private here, so that the compiler can't assume no
	// types are derived from it, otherwise a clever compiler could theoretically optimize away the virtual function
	// calls.
protected:
	// Nested types
	...
	public:
		...
		{
			// Note that this method doesn't strictly need to be virtual, but it is better to make it virtual so that
			// the owning object doesn't have to know anything about the layout of this object. This would allow us to
			// make changes to the structure of this type without breaking compatibility with existing compiled code.
			++_refCount;
		}
		virtual void MARSHALSUPPORT_CALLINGCONVENTION DecrementRefCount()
		{
			// Note that this method must be virtual, so that the assembly which allocated this object will be the one
			// in which the deletion is performed.
			--_refCount;
			if (_refCount <= 0)
			{
				delete this;
