...
public:
	// Constructors
	inline Out(ContainerType& target) // NOLINT(hicpp-explicit-conversions)
	: _targetReference(_target), _target(target)
	{
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
		static_assert(marshalling::internal::is_assignable<ContainerType>::value, "Target of Marshal::Out must be assignable");
#endif
	}
	inline Out(const InOut<ContainerType>& target) // NOLINT(hicpp-explicit-conversions)
	...
	inline const Out& operator=(const ContainerType& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
	...
	inline const Out& operator=(ContainerType&& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(std::move(sourceObject));
		return *this;
	}
...
private:
	...
	Out(const Out& source) MARSHALSUPPORT_DELETEMETHOD;            // NOLINT(hicpp-use-equals-delete,modernize-use-equals-delete)
	Out& operator=(const Out& source) MARSHALSUPPORT_DELETEMETHOD; // NOLINT(hicpp-use-equals-delete,modernize-use-equals-delete)
#ifdef MARSHALSUPPORT_CPP11SUPPORTED
	Out(Out&& source) MARSHALSUPPORT_DELETEMETHOD;            // NOLINT(hicpp-use-equals-delete,modernize-use-equals-delete)
	Out& operator=(Out&& source) MARSHALSUPPORT_DELETEMETHOD; // NOLINT(hicpp-use-equals-delete,modernize-use-equals-delete)
#endif
...
public:
	// Constructors
	inline Out(std::vector<ElementType, Alloc>& target) // NOLINT(hicpp-explicit-conversions)
	: _targetReference(_target), _target(target)
	{}
	inline Out(const InOut<std::vector<ElementType, Alloc>>& target) // NOLINT(hicpp-explicit-conversions)
	: _targetReference(target._targetReference), _target((no_bound_object_tag*)0)
	{}
	...
	inline const Out& operator=(const std::vector<ElementType, Alloc>& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		// Note that we use a reference to the marshaller object here. This is done to ensure that the compiler can't
		// optimize away any virtual function calls we're using to enforce code boundaries between assemblies. If the
		// compiler could be sure of the actual type of the target marshaller object, it could attempt to do this. Since
	...
	inline const Out& operator=(std::vector<ElementType, Alloc>&& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(std::move(sourceObject));
		return *this;
	}
...
public:
	// Constructors
	inline Out(std::array<ElementType, ArraySize>& target) // NOLINT(hicpp-explicit-conversions)
	: _targetReference(_target), _target(target)
	{}
	inline Out(const InOut<std::array<ElementType, ArraySize>>& target) // NOLINT(hicpp-explicit-conversions)
	: _targetReference(target._targetReference), _target((no_bound_object_tag*)0)
	{}
	...
	inline const Out& operator=(const std::array<ElementType, ArraySize>& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		// Note that we use a reference to the marshaller object here. This is done to ensure that the compiler can't
		// optimize away any virtual function calls we're using to enforce code boundaries between assemblies. If the
		// compiler could be sure of the actual type of the target marshaller object, it could attempt to do this. Since
	...
	inline const Out& operator=(std::array<ElementType, ArraySize>&& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(std::move(sourceObject));
		return *this;
	}
...
public:
	// Constructors
	inline Out(std::unique_ptr<T, Deleter>& target) // NOLINT(hicpp-explicit-conversions)
	: _targetReference(_target), _target(target)
	{}
	...
	inline const Out& operator=(std::unique_ptr<T, Deleter>&& sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(std::move(sourceObject));
		return *this;
	}
	inline const Out& operator=(T* sourceObject) const // NOLINT(cppcoreguidelines-c-copy-assignment-signature,misc-unconventional-assign-operator)
	{
		_targetReference.MarshalFrom(std::unique_ptr<T, Deleter>(sourceObject));
		return *this;
	}
