...
class VulkanStateBuffer : public IStateBuffer
{
public:
	// Constructors
	VulkanStateBuffer(cobalt::logging::ILogger* log, VulkanRenderer* renderer, bool isolatedBuffer = false);
	~VulkanStateBuffer();
	...
	// Binding methods
	VkBuffer GetNativeBuffer(uint32_t pageNo);
	size_t GetPageSize();
	size_t GetPageOffset(uint32_t pageNo);

	// State value methods
	StateValueId GetStateValueId(const Marshal::In<std::string>& name) const override;
	void SetRawPageDataWithReturnedPageIndex(uint32_t pageNo, const unsigned char* data, uint32_t& pageIndexInBlock, uint32_t& pageBlockIndex);
	...
	void CompletePendingDataWritesForPageBlock(VkCommandBuffer commandBuffer, uint32_t targetPageBlockNo);
...
private:
	// Constants
	static const size_t ConstantBufferSizeAlignment = 0x10;
	static const size_t ConstantBufferOffsetAlignment = 0x100;

	// Structures
	struct PageBlock
	{
		VkBuffer nativeBuffer;
		VmaAllocation nativeBufferAllocation;
		VkBuffer stagingBuffer;
		VmaAllocation stagingBufferAllocation;
		void* stagingBufferPointer;

		unsigned char* memoryBuffers[2];
...
private:
	...
	static uint32_t GetHighestSetBitNumber(size_t data);
	...
	// Build state methods
	void CompletePendingDataWritesForPageBlock(VkCommandBuffer commandBuffer, PageBlock& pageBlock);
	void FlagBuildStateModified();

private:
	cobalt::logging::ILogger* _log;
	VulkanRenderer* _renderer;
	VulkanStateBufferLayout* _stateBufferLayout = nullptr;
