...
: _log(log), _renderer(renderer)
{
	...
	_buildState.maxLevel = D3D12_FLOAT32_MAX;
...
//----------------------------------------------------------------------------------------
template<class T>
const CD3DX12_GPU_DESCRIPTOR_HANDLE& Direct3DTextureSampler<T>::GetGPUDescriptorHandle() const
{
	// Return the descriptor handle to the caller
	return _samplerHandle->GetNativeGPUHandle();
}

//----------------------------------------------------------------------------------------
template<class T>
void Direct3DTextureSampler<T>::BuildSamplerObject()
{
	bool mipmapDisabled = (_drawState.mipmapMode == ITextureSampler::MipmapMode::None);
	D3D12_FILTER filterMode = GetFilterModeNative(_drawState.filterModeShrink, _drawState.filterModeExpand, _drawState.mipmapMode, _drawState.anisotropyEnabled);
	D3D12_TEXTURE_ADDRESS_MODE wrapModeU = GetWrapModeNative(_drawState.wrapModeHorizontal);
	D3D12_TEXTURE_ADDRESS_MODE wrapModeV = GetWrapModeNative(_drawState.wrapModeVertical);
	D3D12_TEXTURE_ADDRESS_MODE wrapModeW = GetWrapModeNative(_drawState.wrapModeDepth);
	...
	_samplerHandle = _renderer->AllocateDescriptor(Direct3DHeapManager::ResourceType::Sampler);
	_renderer->GetDevice()->CreateSampler(&desc, _samplerHandle->GetNativeCPUHandle());
}
...
void Direct3DTextureSampler<T>::SetAnisotropicFilterModeInternal(bool enabled, int maxAnisotropy)
{
	...
	_buildState.maxAnisotropy = (UINT)maxAnisotropy;
	_buildState.nativeObjectsCurrent = false;
	FlagBuildStateModified();
}
...
//----------------------------------------------------------------------------------------
template<class T>
constexpr D3D12_FILTER Direct3DTextureSampler<T>::GetFilterModeNative(typename T::FilterMode filterModeShrink, typename T::FilterMode filterModeExpand, typename T::MipmapMode mipmapMode, bool anisotropyEnabled)
{
	...
	if ((filterModeShrink == T::FilterMode::Nearest) && (filterModeExpand == T::FilterMode::Nearest))
	{
		...
		{
			return D3D12_FILTER::D3D12_FILTER_MIN_MAG_POINT_MIP_LINEAR;
		}
	...
	if ((filterModeShrink == T::FilterMode::Nearest) && (filterModeExpand == T::FilterMode::Linear))
	{
		if (mipmapMode == T::MipmapMode::Linear)
		{
			return D3D12_FILTER::D3D12_FILTER_MIN_POINT_MAG_MIP_LINEAR;
		}
		return D3D12_FILTER::D3D12_FILTER_MIN_POINT_MAG_LINEAR_MIP_POINT;
	}
	if ((filterModeShrink == T::FilterMode::Linear) && (filterModeExpand == T::FilterMode::Nearest))
	{
		if (mipmapMode == T::MipmapMode::Linear)
		{
			return D3D12_FILTER::D3D12_FILTER_MIN_LINEAR_MAG_POINT_MIP_LINEAR;
		}
		return D3D12_FILTER::D3D12_FILTER_MIN_LINEAR_MAG_MIP_POINT;
	}
	if ((filterModeShrink == T::FilterMode::Linear) && (filterModeExpand == T::FilterMode::Linear))
	{
		...
		return D3D12_FILTER::D3D12_FILTER_MIN_MAG_LINEAR_MIP_POINT;
	}
	UNREACHABLE();
	return {};
}
...
//----------------------------------------------------------------------------------------
template<class T>
constexpr D3D12_TEXTURE_ADDRESS_MODE Direct3DTextureSampler<T>::GetWrapModeNative(typename T::WrapMode wrapMode)
{
	switch (wrapMode)
	{
