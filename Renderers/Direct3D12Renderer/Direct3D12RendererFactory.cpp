...
// Predeclarations
extern "C" __declspec(dllexport) IGraphicsDeviceEnumerator* CreateDirect3D12Renderer(cobalt::logging::ILogger* log);
extern "C" __declspec(dllexport) bool GetDirect3D12RendererPlugin(IRendererPlugin& rendererPlugin);
...
extern "C" __declspec(dllexport) bool GetDirect3D12RendererPlugin(IRendererPlugin& rendererPlugin)
{
	rendererPlugin.SetAllocationFunction(CreateDirect3D12Renderer);
	rendererPlugin.SetApiFamily(IRendererPlugin::ApiFamily::Direct3D);
	rendererPlugin.SetTargetApiVersion(IRendererPlugin::ApiVersion(12, 0));
	rendererPlugin.SetName("Direct3D12_0");
	rendererPlugin.SetDisplayName("Direct3D 12");
	return true;
}
...
extern "C" __declspec(dllexport) bool GetRendererPlugin(unsigned int indexNo, IRendererPlugin& rendererPlugin)
{
	...
	{
	case 0:
		return GetDirect3D12RendererPlugin(rendererPlugin);
	}
	return false;
}
