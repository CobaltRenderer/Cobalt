...
// Constructors
//----------------------------------------------------------------------------------------
OpenGLTransferBatch::OpenGLTransferBatch(cobalt::logging::ILogger* log, OpenGLRenderer* renderer, StartTiming startTiming, EndTiming endTiming)
...
//----------------------------------------------------------------------------------------
void OpenGLTransferBatch::Delete()
{
...
// Submission methods
//----------------------------------------------------------------------------------------
SuccessToken OpenGLTransferBatch::SubmitBatch()
{
...
//----------------------------------------------------------------------------------------
bool OpenGLTransferBatch::IsSubmitted() const
{
	return _isSubmitted;
}

//----------------------------------------------------------------------------------------
bool OpenGLTransferBatch::IsComplete() const
{
	return _isComplete;
}

//----------------------------------------------------------------------------------------
void OpenGLTransferBatch::WaitForComplete() const
{
	std::unique_lock<std::mutex> lock(_completeMutex);
	if (!_isComplete)
	{
...
//----------------------------------------------------------------------------------------
void OpenGLTransferBatch::IncrementUsageCount()
{
	std::unique_lock<std::mutex> lock(_completeMutex);
	++_usageCount;
}
...
//----------------------------------------------------------------------------------------
void OpenGLTransferBatch::DecrementUsageCount()
{
	std::unique_lock<std::mutex> lock(_completeMutex);
	if (--_usageCount == 0)
	{
...
// Submission methods
//----------------------------------------------------------------------------------------
void OpenGLTransferBatch::NotifyComplete()
{
	_isComplete = true;
