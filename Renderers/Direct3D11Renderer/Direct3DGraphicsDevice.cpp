...
// Constructors
//----------------------------------------------------------------------------------------
Direct3DGraphicsDevice::Direct3DGraphicsDevice(cobalt::logging::ILogger* log, Microsoft::WRL::ComPtr<IDXGIFactory2> dxgiFactory, Microsoft::WRL::ComPtr<IDXGIAdapter2> adapter, const DXGI_ADAPTER_DESC2& adapterDescription)
...
Direct3DGraphicsDevice::ImageLimits Direct3DGraphicsDevice::GetImageLimits() const
{
	...
	imageLimits.maxImageDimensionTexture1D = D3D11_REQ_TEXTURE1D_U_DIMENSION;
	imageLimits.maxImageDimensionTexture2D = D3D11_REQ_TEXTURE2D_U_OR_V_DIMENSION;
	imageLimits.maxImageDimensionTexture3D = D3D11_REQ_TEXTURE3D_U_V_OR_W_DIMENSION;
	imageLimits.maxImageDimensionTextureCube = D3D11_REQ_TEXTURECUBE_DIMENSION;
	imageLimits.maxImageArraySizeTexture1D = D3D11_REQ_TEXTURE1D_ARRAY_AXIS_DIMENSION;
	imageLimits.maxImageArraySizeTexture2D = D3D11_REQ_TEXTURE2D_ARRAY_AXIS_DIMENSION;
	imageLimits.maxSamplerAnisotropicFilteringLevel = D3D11_REQ_MAXANISOTROPY;
	return imageLimits;
}
...
Direct3DGraphicsDevice::ShaderLimits Direct3DGraphicsDevice::GetShaderLimits() const
{
	...
	shaderLimits.maxVertexShaderInputAttributes = D3D11_VS_INPUT_REGISTER_COUNT;
	shaderLimits.maxVertexShaderOutputComponents = D3D11_VS_OUTPUT_REGISTER_COUNT * D3D11_VS_OUTPUT_REGISTER_COMPONENTS;
	shaderLimits.maxGeometryShaderInputComponents = D3D11_GS_INPUT_REGISTER_COUNT * D3D11_GS_INPUT_REGISTER_COMPONENTS;
	shaderLimits.maxGeometryShaderOutputComponents = D3D11_GS_OUTPUT_REGISTER_COUNT * D3D11_GS_OUTPUT_REGISTER_COMPONENTS;
	shaderLimits.maxGeometryShaderOutputVertices = D3D11_GS_MAX_OUTPUT_VERTEX_COUNT_ACROSS_INSTANCES;
	shaderLimits.maxGeometryShaderTotalOutputComponents = D3D11_GS_MAX_OUTPUT_VERTEX_COUNT_ACROSS_INSTANCES * D3D11_GS_OUTPUT_REGISTER_COMPONENTS;
	shaderLimits.maxFragmentShaderInputComponents = D3D11_PS_INPUT_REGISTER_COUNT * D3D11_PS_INPUT_REGISTER_COMPONENTS;
	return shaderLimits;
}
...
Direct3DGraphicsDevice::DrawLimits Direct3DGraphicsDevice::GetDrawLimits() const
{
	...
	drawLimits.maxVertexCountPerDraw = (uint32_t)((1ull << D3D11_REQ_DRAW_VERTEX_COUNT_2_TO_EXP) - 1ull);
	drawLimits.maxIndexValue = (uint32_t)((1ull << D3D11_REQ_DRAWINDEXED_INDEX_COUNT_2_TO_EXP) - 1ull);

	drawLimits.maxResourcesPerDraw = D3D11_COMMONSHADER_INPUT_RESOURCE_REGISTER_COUNT;
...
Direct3DGraphicsDevice::FrameBufferLimits Direct3DGraphicsDevice::GetFrameBufferLimits() const
{
	...
	frameBufferLimits.maxFrameBufferWidth = D3D11_VIEWPORT_BOUNDS_MAX;
	frameBufferLimits.maxFrameBufferHeight = D3D11_VIEWPORT_BOUNDS_MAX;
	frameBufferLimits.maxFrameBufferColorAttachments = D3D11_PS_OUTPUT_REGISTER_COUNT;
...
Direct3DGraphicsDevice::DataBufferLimits Direct3DGraphicsDevice::GetDataBufferLimits() const
{
	...
	limits.maxStateBufferPageSize = D3D11_REQ_CONSTANT_BUFFER_ELEMENT_COUNT * sizeof(V4Float32);
