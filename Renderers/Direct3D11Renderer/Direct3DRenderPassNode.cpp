...
{
	// There's nothing we can do under the Direct3D 11 API to optimize around attachment load/store behaviour, so we
...
//----------------------------------------------------------------------------------------
void Direct3DRenderPassNode::ApplyFixedState(ID3D11Device1* device, ID3D11DeviceContext1* context)
{
	...
	for (const auto& entry : _drawState.attachmentState)
	{
		...
		case IFrameBuffer::AttachmentType::Color:
			_drawState.framebuffer->ClearRenderView(context, entry.index, entry.clearData);
			break;
	...
	{
		_drawState.framebuffer->ClearDepthStencilView(context, depthClearData, stencilClearData);
	}
}

//----------------------------------------------------------------------------------------
void Direct3DRenderPassNode::PerformUnbindOperations(ID3D11DeviceContext1* context)
{
	...
	for (const auto& attachmentState : _drawState.attachmentState)
	{
		...
		_drawState.framebuffer->ResolveMultiSamplingAttachmentToTexture(context, attachmentState.type, attachmentState.index, attachmentState.multiSampleResolveTargetIndex);
	}
}
